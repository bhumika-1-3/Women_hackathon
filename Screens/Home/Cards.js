...
const Cards = ({ description, name, price, image_url, store_name }) => {
    ...
    return (
        <View className="p-5 my-5 bg-white dark:bg-gray-50/10 w-full rounded-3xl">
            <View className="bg-white rounded-3xl"><Image
                source={{ uri: image_url }} className="w-full h-72" style={{ resizeMode: "contain" }}></Image></View>
            <View className="mt-5">

                <Text className="text-slate-600 dark:text-zinc-300 text-sm">@{store_name}</Text>
                <Text className="text-slate-900 dark:text-zinc-200 text-base font-bold mb-8">{name}</Text>
                ...
                <TouchableOpacity onPress={() => alert("Added Successfully")} className="flex-row bg-rose-500 dark:bg-zinc-200 p-4 rounded-full w-10/12 self-center justify-center">
